...
describe PostsForBranchService do
  context '#call' do
    ...
    it 'returns posts filtered by a branch' do
      ...
      expect(PostsForBranchService.new({branch: 'hobby'}).call).
    ...
    it 'returns posts filtered by a branch and a search input' do
      ...
      category
      included_post = [] << post
      expect(PostsForBranchService.new({branch: 'hobby', search: 'fun'}).call).
      to(eq included_post)
    end
    ...
    it 'returns posts filtered by a category name' do
      ...
      category
      included_post = [] << post
      expect(PostsForBranchService.new({branch: 'hobby', category: 'arts'}).call).
      to(eq included_post)
    end
